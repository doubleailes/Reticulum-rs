...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let peer_dest = RatchetedDestination::new(
        peer_identity,
        peer_dest_name,
        Some(temp_dir.clone()),
    )?;
                ...
                match ratcheted_dest.decrypt(OsRng, encrypted, &mut decrypt_buf) {
                    Ok(decrypted) => {
                        if decrypted == test_message {
                            println!("✅ Self-encryption test {} passed ({} bytes)", i + 1, test_message.len());
                        } else {
        ...
        match ratcheted_dest.announce(OsRng, None) {
            Ok(_) => {
                ...
                if current_keys > initial_keys + i {
                    println!("✅ Key rotation {} successful ({} keys available)", i + 1, current_keys);
                } else {
                    println!("⚠️  Key rotation {} may not have increased key count", i + 1);
                }
                        ...
                        match ratcheted_dest.decrypt(OsRng, encrypted, &mut decrypt_buf) {
                            Ok(decrypted) => {
                                if decrypted == test_msg.as_bytes() {
                                    ...
                                    println!("❌ Post-rotation encryption {} failed: data mismatch", i + 1);
    ...
    match ratcheted_dest.create_data_packet(OsRng, test_data) {
        Ok(packet) => {
            println!("✅ Data packet created successfully ({} bytes)", packet.data.len());
    ...
    let reloaded_dest = RatchetedDestination::new(
        reload_identity,
        dest_name,
        Some(temp_dir.clone()),
    )?;
    ...
    if keys_after_reload > 0 {
        println!("✅ Keys persisted and reloaded ({} keys recovered)", keys_after_reload);
    ...
    match reloaded_dest.encrypt(OsRng, test_msg, &mut encrypt_buf) {
        Ok(encrypted) => {
            match reloaded_dest.decrypt(OsRng, encrypted, &mut decrypt_buf) {
                Ok(decrypted) => {
    ...
    if score >= 7 {
        println!();
        println!("🚀 SUCCESS: Your Rust implementation demonstrates comprehensive ratchet compatibility!");
        ...
        println!();
        println!("⚠️  PARTIAL SUCCESS: Basic network functionality works but ratchet compatibility has issues.");
        println!("📝 Your implementation can connect to the network but may not be fully compatible with Python RNS ratchets.");
    } else {
        println!();
