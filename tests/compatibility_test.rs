...
mod compatibility_tests {
    ...
    async fn test_ratchet_file_format_compatibility() {
        ...
        let file_contents = fs::read(&expected_file).expect("Should be able to read ratchet file");
        ...
        assert!(
            ...
            "Ratchet file should not be empty"
        );
        ...
        if !file_contents.is_empty() {
            ...
            assert!(
                first_byte >= 0x80,
                "First byte should indicate MessagePack format"
            );
        }
    ...
    async fn test_key_rotation_compatibility() {
        ...
        assert_eq!(
            keys_after_1.len(),
            1,
            "Should have 1 key after first announce"
        );
        ...
        for i in 2..=5 {
            ...
            assert_eq!(
                keys.len(),
                i,
                "Should have {} keys after {} announces",
                i,
                i
            );
        }
        ...
        if test_vectors_path.exists() {
            ...
            if let Some(test_cases) = test_vectors["test_cases"].as_array() {
                for (i, test_case) in test_cases.iter().enumerate() {
                    ...
                    let identity_hash = test_case["identity_hash"]
                        .as_str()
                        .expect("identity_hash should be string");
                    let destination_hash = test_case["destination_hash"]
                        .as_str()
                        .expect("destination_hash should be string");
                    let public_key = test_case["public_key"]
                        .as_str()
                        .expect("public_key should be string");

                    // Validate hex format
                    assert!(
                        hex::decode(identity_hash).is_ok(),
                        "identity_hash should be valid hex"
                    );
                    assert!(
                        hex::decode(destination_hash).is_ok(),
                        "destination_hash should be valid hex"
                    );
                    assert!(
                        hex::decode(public_key).is_ok(),
                        "public_key should be valid hex"
                    );
